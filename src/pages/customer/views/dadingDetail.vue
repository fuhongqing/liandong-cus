<template>
  <div class="baobei" v-if="khdata">
    ...
    <div class="detail">
      <div><span class="label">认购日期:</span>{{khdata.ddTime||''}}</div>
      <div><span class="label">认购总价:</span>{{khdata.totalPrice||''}}元</div>
      <div><span class="label">认购金额:</span>{{khdata.price||''}}元</div>
      <div><span class="label">认购面积:</span>{{khdata.dadingArea}}m²</div>
      <div><span class="label">认购书:</span>
        <p class="image"><img width="100" height="76" v-for="(item,index) in khdata.rg_img.split(',')" :key="index"
                              @click="show(item)" :src=" item + '?imageView2/1/w/103/h/76'"/></p>
      </div>
      <div><span class="label">认购收据:</span>
        <p class="image"><img width="100" height="76" v-for="(item,index) in khdata.dd_img.split(',')" :key="index"
                              @click="show(item)" :src=" item + '?imageView2/1/w/103/h/76'"/></p>
      </div>
      <div><span class="label">认购POS单:</span>
        <p class="image"><img width="100" height="76" v-for="(item,index) in khdata.pos_img.split(',')" :key="index"
                              @click="show(item)" :src="item + '?imageView2/1/w/103/h/76'"/></p>
      </div>
    </div>
    <div class="title">
      ...
      getkhdata() {
        this.$axios.get(this.khApi + "/customer/v1/findCustomerInfoById?type=" + this.type+'&id='+this.id).then((res) => {
          if (res.data.code == "200") {
            this.khdata = res.data.data.dadingInfo;
